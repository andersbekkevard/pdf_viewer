...
# Optional parameters:
# @raycast.icon 📑
# @raycast.description Bulk-convert every *.pdf under a folder into the pdf_viewer cache. Idempotent (already-cached docs are skipped).
...
# inherit them from the user's interactive shell — `fd` and the native
# pdf2htmlEX binary's deps live there. The underlying script fails fast with
# an install hint (via macOS notification) if the native binary is missing.
